...
<div style="padding: 2rem;">
    <h2 style="color: #047857; margin-bottom: 1.5rem;">{{ isset($galeri) ? '✏️ Edit' : '➕ Tambah' }} Gambar Galeri</h2>
    
    <form action="{{ isset($galeri) ? route('admin.galeri.update', $galeri) : route('admin.galeri.store') }}" method="POST" enctype="multipart/form-data" style="background: white; padding: 2rem; border-radius: 10px; max-width: 800px; margin-top: 2rem; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        @csrf
        @if(isset($galeri))
            @method('PUT')
        @endif
        ...
        <div style="margin-bottom: 1.5rem;">
            <label style="display: block; margin-bottom: 0.5rem; font-weight: 600; color: #047857;">📌 Judul Gambar</label>
            <input type="text" name="judul" value="{{ old('judul', $galeri->judul ?? '') }}" required
                ...
                placeholder="Contoh: Suasana Pagi di Kebun">
            @error('judul')
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            @endif
            <input type="file" name="gambar" accept="image/*" {{ !isset($galeri) ? 'required' : '' }}
            ...
            <small style="color: #6b7280; font-size: 0.875rem; display: block; margin-top: 0.5rem;">
                📏 Ukuran maksimal: 5MB • Format: JPG, PNG, GIF • Resolusi rekomendasi: 1200x900px (4:3)
            </small>
            @error('gambar')
                <span style="color: #dc2626; font-size: 0.875rem; display: block; margin-top: 0.5rem;">{{ $message }}</span>
            @enderror
        ...
        <div style="display: flex; gap: 1rem; margin-top: 2rem; padding-top: 1.5rem; border-top: 2px solid #f3f4f6;">
            ...
            <a href="{{ route('admin.galeri.index') }}" style="background: #6b7280; color: white; padding: 0.85rem 2rem; text-decoration: none; border-radius: 8px; display: inline-flex; align-items: center; font-weight: 600; transition: background 0.3s;"
